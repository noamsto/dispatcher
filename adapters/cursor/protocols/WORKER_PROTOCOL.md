...
1. **Write the artifact** into the crew dir (from `WORKER_TASK.md`):
   ...
   resolved roster beside it: `reviewer-roster --base <base> >
   <crew_dir>/artifacts/<branch>/roster.json` — or, if the resolver is
   unavailable, skip `roster.json` and state
   `repo-local discovery skipped: <reason>` in the assignment instead.
...
- **Dispatch the review as a single parallel batch** — one message, concurrent subagents — then reconcile once. Do not run reviewers serially. The roles are identical on every engine; the engine only decides how you spawn one, and which rung reviews:
  ...
  | ------ | ------------------ | ------------- |
  | **claude** | Agent tool, one subagent per matched roster entry, its body as the brief; a native agent is preferred only for a harness identity — the entry's `name` when `source` is `harness`, or `override.of` when set — matched by that name or one of that harness entry's `aliases:`, and it is spawned with the resolved brief; an entry with `override: null` always runs as a general subagent with its brief | unchanged — each agent definition owns its model |
  ...
  Every row inlines the resolver's `brief` for the entry — for a harness entry that is its body, for a repo-local entry the framed brief.
