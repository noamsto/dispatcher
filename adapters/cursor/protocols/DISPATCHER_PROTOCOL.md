...
```
dispatch <tier> <model> --effort <low|medium|high|xhigh|max|ultra> [--agent claude|codex|cursor|pi] [--mcp <profile>] [--grid] [--roles <role[=model|agent:model][@effort],…>] [--plan provided|required] [--base <ref>] [--pr N] [--review] [LINEAR-ID] <title…>
```
...
- **Stacked base.** `--base <ref>` starts the worker's branch at the pinned oid of `origin/<ref>` — fetched before the dispatch lock, so an unresolvable ref costs no worktree and no window — and stamps `base: <ref>` in `WORKER_TASK.md`. The worker then diffs, gates, and opens its PR against that parent branch instead of the default branch, which is the whole point of stacked work: the target files exist only on an open PR's branch. `--base` cannot combine with `--pr` (which already fixes the base from the PR's `baseRefName`). Omit it for the unchanged default-branch behaviour, where no `base:` is stamped.
