...
# does not match this script's baked revision (#184). The build substitutes the
# content hash of adapters/core/protocols (minus PROTOCOL_REV itself) for
# @protocolRev@; a matching dir carries the same value in a committed
# PROTOCOL_REV file (generated by scripts/gen-adapters.sh). A stale dir — an
# old store path held by a long-lived DISPATCHER_PROTOCOL_DIR export — has an
# absent or older PROTOCOL_REV and is refused before any scaffolding. A raw
# checkout script (marker unsubstituted) cannot bind a revision and skips with
# a one-line warning.
_check_protocol_rev() {
  local dir="$1" label="$2" stamped_rev="@protocolRev@" rev_file dir_rev
  ...
  if [[ "$stamped_rev" == @* ]]; then
    echo "$label: unsubstituted protocol revision (raw checkout script) — skipping the PROTOCOL_REV consistency check" >&2
    return 0
  fi
  rev_file="$dir/PROTOCOL_REV"
  if [ ! -f "$rev_file" ]; then
    echo "$label: \$PROTOCOL_DIR ($dir) carries no PROTOCOL_REV — a stale protocol directory; refusing to launch" >&2
    [ -n "${DISPATCHER_PROTOCOL_DIR:-}" ] && echo "$label:   DISPATCHER_PROTOCOL_DIR override: $DISPATCHER_PROTOCOL_DIR (unset it, or start a fresh shell so the export refreshes)" >&2
    echo "$label:   a checkout is brought up to date by scripts/gen-adapters.sh" >&2
    exit 1
  fi
  dir_rev="$(<"$rev_file")"
  if [ "$dir_rev" != "$stamped_rev" ]; then
    ...
    echo "$label:   \$PROTOCOL_DIR/PROTOCOL_REV:   $dir_rev" >&2
