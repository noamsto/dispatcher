...
- **Stacked base.** `--base <ref>` starts the worker's branch at the pinned oid of `origin/<ref>` — fetched before the dispatch lock, so an unresolvable ref costs no worktree and no window — and stamps `base: <ref>` in `WORKER_TASK.md`. The worker then diffs, gates, and opens its PR against that parent branch instead of the default branch, which is the whole point of stacked work: the target files exist only on an open PR's branch. `--base` also takes a **PR number**: an OPEN, same-repo PR resolves to its `headRefName`, which is what actually gets stamped and fetched. `--base` cannot combine with `--pr` (which already fixes the base from the PR's `baseRefName`). Omit it for the unchanged default-branch behaviour, where no `base:` is stamped. Each layer is its own tracked task — its own issue/ticket, its own worker, its own PR; the GitHub claim gate refuses a second layer on an already-claimed issue, and the three-way duplicate guard (above) is otherwise unchanged.
  ...
  - **Rebase coordination.** When a lower layer changes — new commits pushed, or merged — direct only its **immediate** child to rebase its own branch. Any `DISPATCH_SPEC` replaces the carried task body entirely (dispatch carries the old `## Task` only when `DISPATCH_SPEC` is unset), so a `DISPATCH_SPEC` used to deliver a rebase directive must carry the task text plus the directive, never the directive alone. The directive itself names the new base, and — after a squash-merge — the cut point (the merged parent's `headRefOid`, from `gh pr view <parent> --json headRefOid`) and the parent PR number, so the worker can fetch the cut commit (`git fetch origin pull/<parent-PR>/head`) if it isn't local yet. That fresh `gh pr view <parent> --json headRefOid` is the squash-merge cut when the child was already rebased onto the parent's last push. If the child was not, the cut is the parent's recorded old head, not the fresh view. Every rebase directive carries that recorded old head, including a fast-forward `git rebase origin/<parent>`, so the child can run the ancestor gate. Before directing any layer to rebase, record that layer's `headRefOid` (`gh pr view <layer> --json headRefOid`) and keep the oid — after that layer force-pushes, a later `gh pr view` returns the rewritten tip and the old head is unrecoverable. Direct the immediate child from that record, not from a fresh view of the parent. When the parent's push rewrote history — `git merge-base --is-ancestor <recorded old head> origin/<parent>` fails — direct the child with `git rebase --onto origin/<parent> <recorded old head>`. Plain `git rebase origin/<parent>` is only for a fast-forward advance, and only when that ancestor check succeeds. In a stack main←A←B←C, A squash-merging makes B rebase `--onto` and force-push B′; C must then be directed with `git rebase --onto origin/B <B's recorded old head>`, because a plain `git rebase origin/B` replays A's commits onto B′, which already contains them. Three paths, by the child's state: **live** → `crew reply` with the directive; **finished, has a PR** → `dispatch … --pr <child PR>` with a `DISPATCH_SPEC` carrying the task text plus the directive — here the base comes from the PR itself (dispatch refuses `--base` with `--pr`), so the directive is what names the new base for the worker's own rebase and `gh pr edit --base`; **finished, no PR** → a same-title re-dispatch with a `DISPATCH_SPEC` carrying the task text plus the directive, always passing `--base <new base>` explicitly — a carried `base:` is only right when the parent merely advanced, not on a fresh re-dispatch. The worker applies the directive by rewriting its own `base:` line and retargeting its own PR (see **Base ref (stacked work)** in `WORKER_PROTOCOL.md`). Wait for that child to push before directing the next layer up. Never a cascading `gh stack rebase`. A worker that blocks because its parent branch is gone gets this same directive.
