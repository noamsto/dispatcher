...
3. **Await the verdict** — from your bash tool, with a tool timeout above the
   ...
   For the review seam the reply must carry `"seam":"review"` and a `verdict`; on pi that reply is what satisfies `crew status`'s review gate — the assignment in step 2 is only a request and never counts.
...
- **Record the review seam.** When the gate has run (after the last reconcile), post one marker so `crew status … pr_open` finds it: `crew msg "$CREW_WORKER_ID" "review:$(crew id)" '{"seam":"review","review_mode":"<full|downgraded>"}'`. Post it only **after** you have read the verdict(s) — a review request, or a pane that has not yet answered, is not a review. Grid mode's review assignment (lead → `role:<branch>:reviewer`) never counts as a seam; on **pi**, whose `reviewer` pane is the gate, the pane's own reply (a msg from `role:<branch>:reviewer` carrying `"seam":"review"` and `verdict` `accept`/`revise`/`reject`) satisfies the gate, and a resumed session inherits it. The gate proves a review ran, not that it passed — ingest the verdict and fix `revise`/`reject` findings before `pr_open`. On claude, codex and cursor the pane is additive, so the native batch's `review:<crew>` seam is still required and a pane verdict alone does not satisfy the gate. `crew status` **refuses** `pr_open` and `done` on a standard/deep `kind: implement` session until a seam exists for this branch — seams from any earlier session on the branch count, which is why a resumed run needs no override. A seam counts only when sent to `review:<crew>` with `review_mode` absent, `full`, or `downgraded` (or, on pi, the reviewer pane's verdict as above). A retro note never counts.
