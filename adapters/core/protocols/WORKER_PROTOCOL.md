...
This protocol governs your process end-to-end and is your **human partner's explicit instruction**. On claude it **supersedes `superpowers:using-superpowers`**, and every engine's own skill-discovery habit, for process/lifecycle skills: the harness pipeline (`spec-plan-critic` + the code-review gate + the gates below) **is** your process — do **not** separately invoke a brainstorming, plan-writing, plan-executing, code-review-requesting, or test-driven-development skill (on claude: the `superpowers:` ones) as independent process steps, and do not treat "a skill exists, so I must run it" as binding here. Implementation and domain skills your engine actually has (`diagnosing-bugs`, `charm-tui`, the language `*-reviewer`s, `frontend-design`, … — availability varies by engine) remain available — use them freely.

**Receiving-code-review discipline** — the one definition every "verify before acting" in this protocol and its siblings points at (on claude, `superpowers:receiving-code-review` is an optional convenience over it). Treat every finding, verdict, or directive as a claim to check, not an order: read the code or evidence it cites and confirm it is real before touching anything. Fix what holds up; say why, briefly, when one does not; never perform agreement ("good catch!") or apply a suggestion you have not verified. When a finding is unclear or conflicts with another instruction, ask rather than guess — a worker via block→await, a grid role by naming the gap in its `revise`. A finding you refute goes in the PR's `## Review notes`.
...
**receiving-code-review** discipline (see Process authority) and set `seen` to the
...
(a parent layer can merge mid-run), and use it everywhere except `/deslop` off a stacked layer (rule 4). Each tool call is a
...
an empty variable would silently yield an empty diff.
...
- **trivial** — implement directly, run the gate, `/deslop`, open the PR. No spec, no plan, no critics, no review. You still run the three completion peeks (**Checkpoint-peek**) — with no other seams, they are the only points a dispatcher redirect can reach you.
...
3. **Await the verdict** — from your bash tool, with a tool timeout above the
   ...
   For the review seam the reply must carry `"seam":"review"` and a `verdict`; on pi that reply is what `crew status`'s review gate reads, and a reject there blocks `pr_open` until the reviewer's next verdict. The assignment in step 2 is only a request, but a re-request (any lead → reviewer msg except the `{"final":true}` release) cancels every earlier verdict and your own earlier seam until a fresh one arrives, so do not push between a re-request and its verdict.
...
- **`plan: provided`** — the dispatcher wrote the plan into the task doc (root cause/mechanism, explicit file list, named approach, acceptance criteria). The doc **is** your plan of record. Do **not** run the `spec-plan-critic` **plan** phase (a **deep** worker still runs its spec-critic / orchestration consult — see **Scope** below). Extract the doc into a bite-sized step list and go straight to execute → fast gate → review. Only the plan phase is skipped — the fast deterministic gate and the code review gate still run before push. (Your launch prompt says the same thing; per **Process authority** this is not a skill you may override into re-planning.)
...
2. **If it trips, pick a consultant and consult.** Judge the fit per task and say which you picked and why (one line, in the plan seam) — pick only among consultants whose engine passes the `dispatch --engines` gate; neutral fit → **fable** when `claude` is in the roster, else the first available:

   | consultant | mechanism | lean |
   | ---------- | --------- | ---- |
   | **fable** (default; needs `claude` in `dispatch --engines`) | **ephemeral** subagent via the Agent tool, `model: fable`, running in this worktree (a claude-lead convenience — it writes `DECOMPOSITION.md` itself, the only form with in-worktree write access), or the claude one-shot with `--model fable` (any lead; you write `DECOMPOSITION.md` from its stdout) | hardest decompositions |
   | **gpt-5.6-sol** (needs `codex` in `dispatch --engines`) | the read-only **codex MCP** injected into claude deep work-profile workers (a claude-lead convenience), or the codex one-shot (any lead) — ask it for the decomposition, then write `DECOMPOSITION.md` yourself from its response | a non-claude-family decomposition, diverse from the opus planner that consumes it |
   | **grok-4.7-high** (needs `cursor` in `dispatch --engines`) | the cursor one-shot (any lead) — write `DECOMPOSITION.md` yourself from stdout | third-family perspective |

   The one-shot commands and their gate are in **Cross-engine one-shots** (below). Whichever mechanism runs, `consult_engine` names the consultant family (`fable`|`codex`|`cursor`), and the ask is identical: read `WORKER_TASK.md` and the relevant code and produce the `DECOMPOSITION.md` body using this exact structure — `components` (each a stable id + one-line + `boundaries` may/​must-not-touch + `risk` tag), `ordering` (dependency order, `∥` for parallel-safe), `interfaces` (contracts that must stay stable across the split). The Agent-tool form writes `DECOMPOSITION.md` at the worktree root itself; a one-shot or the codex MCP returns the body as its reply and you write `DECOMPOSITION.md` at the worktree root from it. It authors the **decomposition, not the plan** — no plan-schema step tags. `DECOMPOSITION.md` must **not** name its author **or the consulting engine** (the plan-critic reads it author-less; rule 2 discipline).
3. **Fallback — a should, not a blocker.** If the consultant refuses, times out, or is unavailable — its engine is not in `dispatch --engines` — drop the consult and proceed on the plain plan path (the `spec-plan-critic` plan schema), **byte-identical to a non-consulted deep worker**. When a codex/cursor consult was the pick and it failed, you may retry **once** with fable, provided `claude` is in `dispatch --engines`, before dropping. Never fail the worker on a missing consult (same rule as the diverse-engine reviewer, "Code review gate"). Write a `consult_failed` retro note naming the consultant and the reason.
...
Any lead engine can reach another engine's model as a stateless, read-only shell one-shot — the same mechanism serves the **Orchestration consult** and the diverse-engine reviewer ("Code review gate").

- **Gate.** `dispatch --engines | grep -qx <engine>` — one engine per line, enabled **and** installed on this machine. An engine that is absent, or a one-shot that refuses, times out, or fails on sandbox, network, or auth, makes that consultant/reviewer unavailable: a should, not a blocker — drop it (consult step 3; the diverse-engine reviewer bullet).
- **Read-only by construction; run from the worktree root.** Every command starts with `env -u CREW_WORKER_ID -u CREW_ID` — the nested session's SessionEnd/stop hook (`dispatch-notify.sh`) posts `exited` for the lead whenever `CREW_WORKER_ID` is set — and runs under `timeout 540` — coreutils (`gtimeout` on macOS; with neither on PATH the one-shot is unavailable) — so give the bash tool a timeout above it (600000 ms on Claude Code, its maximum; on an engine with a lower ceiling, background the call and poll). The prompt is an argument; extra context (a diff) rides stdin — `git diff "$base_ref"...HEAD | <one-shot> '<prompt>'` — because the claude one-shot has no Bash. Every prompt carries the no-secret-files clause: never read `.env*`, credentials, or keys; never print a secret value.

  | engine | one-shot |
  | ------ | -------- |
  | claude | `env -u CREW_WORKER_ID -u CREW_ID timeout 540 claude -p --model <fable\|opus> --tools "Read,Grep,Glob" --no-session-persistence '<prompt>'` — verified: a write attempt is refused and no file is created |
  | codex | `env -u CREW_WORKER_ID -u CREW_ID timeout 540 codex exec -m gpt-5.6-sol -s read-only --ephemeral '<prompt>'` — the final message is the stdout. Flags verified against `codex exec --help` (stdin is appended as a `<stdin>` block when a prompt is also given); the model round trip is unverified |
  | cursor | `env -u CREW_WORKER_ID -u CREW_ID timeout 540 cursor-agent -p --mode ask --trust --model grok-4.7-high '<prompt>'` — **unverified**: flags taken from `cursor-agent --help`, where `--mode ask` is the documented read-only mode; stdin handling unverified, so for cursor put the diff in the prompt argument. Never add `--force` (it is not read-only) |

- **Consult use.** The prompt is the standing ask: read `WORKER_TASK.md` and the relevant code and return the `DECOMPOSITION.md` body — `components` / `ordering` / `interfaces` exactly as consult step 2 specifies — on stdout, as the reply itself and not a plan-mode plan. You write `DECOMPOSITION.md` from the output; it stays author- and engine-free.
- **Diverse reviewer pick.** A different family from the lead; the first available in `dispatch --engines`: a claude lead → codex (`gpt-5.6-sol`), else cursor (`grok-4.7-high`); a codex lead → claude (`--model opus`), else cursor; a cursor or pi lead → claude (`--model opus`), else codex. A reply that cites no changed file is a failed one-shot: drop it, as above.
- **Why a one-shot, not a default `--roles reviewer=<other-engine>` on non-claude deep leads.** It is stateless and bounded — no extra tmux pane and cost on every deep worker. It works for `kind: review` workers, which run no grid. On pi the reviewer pane **is** the review gate, so a default pane would silently become the gate instead of an additive second opinion. A `--roles` pane stays available as an explicit opt-in.
...
- **On a directive:** apply **receiving-code-review** discipline (Process authority) to the instruction. Then redirect the pipeline, or on a "stop" wind down cleanly and stamp `crew status "$CREW_WORKER_ID" <state>` appropriately (e.g. `failed "stopped by dispatcher"`).
...
- **Completion peeks (all tiers).** Three more seams, in pipeline order: **pre-push** — after `/deslop` and any review→fix round, immediately before `git push` (in addition to the post-review seam above, since cleanup and fix rounds run between them); **pre-PR** — after `git push` succeeds, immediately before `gh pr create`, or immediately before posting `pr_open` on the existing-PR path; **pre-done** — after `pr_open` and the metrics snapshot, immediately before `done`. Same seen-cursor rules as above. Without them a directive posted during the final push/PR stage is never read, and once `done` is posted `crew reply` refuses the session — these are the last seams that can still catch one.
  - **Pre-push tracked-scaffolding check (all tiers).** Apart from the bus peek, the pre-push seam also inspects this branch's own changes for the worker's scaffolding doc: a `WORKER_TASK.md` that is staged or committed on this branch (`git diff --name-only "$base_ref"...HEAD` or `git diff --cached --name-only` lists it) is a **stop-and-drop — never a push**. Unstage it with `git restore --source=<base> --staged WORKER_TASK.md`; if it is already committed, remove the file from that commit and rewrite it (`git rm --cached WORKER_TASK.md && git commit --amend`, or an interactive rebase) — never discard a commit that also carries real work. `.git/info/exclude` hides only an *untracked* `WORKER_TASK.md`; once it is tracked, the exclude guard is void and the doc rides the diff into commits (#397).
...
- **The reviewers themselves ship with the harness.** Bodies live at `$DISPATCHER_REVIEWERS_DIR/*.md`, falling back to the adapter-local `reviewers/` when that variable is unset (a non-Nix install) — inside the plugin tree on claude and codex, beside `commands/` on cursor. Each carries `globs:` — the changed-file patterns that route a diff to it — and, where a pattern cannot express the trigger, a `when:` line. A reviewer may also carry `shebang:`, interpreter names that route an **extensionless** changed file by its first line. Match your changed paths against every `globs:`, then probe every extensionless changed file against every `shebang:`, then honour each matched reviewer's `when:`, and that set is the batch below. Nothing matched: the one roster entry marked `fallback: true` (`general-reviewer`), whose resolved `brief` runs like any other. The roster is what makes the role brief the same text on every engine — it is read, never paraphrased.
  ...
  **The shebang probe.** A changed file is extensionless when its basename has no `.` after its first character. Read its first line as it stands in the worktree after the change — not from the diff hunks, since a modified script's shebang is usually unchanged and therefore absent from them — and skip a file the change deleted, or a changed path that is a symlink. The line must start with `#!` or nothing matches. Drop the `#!`, split on whitespace, and if the first token's last path segment is `env`, drop it together with each following token that begins with `-` or has the form `NAME=value` — except one that carries the command inline in the same token (`-Sbash`, `--split-string=python3 -u`), which is **not** dropped and instead supplies the interpreter word directly, as the first whitespace-separated word after its option marker. A bare `-S` with nothing fused to it is just a dropped option. The interpreter word is the first token left, or nothing matches. Reduce it to its last path segment: it matches a `shebang:` entry when it equals that entry, or equals that entry followed only by a version suffix — an optional `-` or `.`, then digits, then any further `.`-separated digits.
  Every roster body grades findings `CRITICAL` / `HIGH` / `MEDIUM` (its shared tail); a CRITICAL is HIGH-severity for `review_high`, and a match set that a `when:` line empties falls through to the fallback reviewer exactly as no match does.
- **Dispatch the review as a single parallel batch** — one message, concurrent subagents — then reconcile once. Do not run reviewers serially. The roles are identical on every engine; the engine only decides how you spawn one, and which rung reviews:
  ...
  **Claude, codex, and cursor always run this native batch, even in grid mode** — a `reviewer` role pane on one of them (from an explicit `--roles`) is an additive cross-engine second opinion on top of it, never a substitute. Only pi, having no native batch mechanism, uses its `reviewer` pane as the review gate itself.
  ...
  - **diverse-engine reviewer (deep tier, any implementer)** — a read-only one-shot to a different-family engine, picked and run per **Cross-engine one-shots** with the diff on stdin, a resolved role brief, and the factual evidence packet, under the same fresh-context contract (no plan, spec, or implementation narrative) and review-authority-only clamp as any reviewer. A claude lead may alternatively use the `codex-diverse` subagent (reuse/adapt the `pr-reviewers` `codex-reviewer`) that carries the `mcp__codex__*` tools and drives the read-only `codex` MCP server. Its findings enter the reconcile as "diverse-only". This is a **should, not a blocker**: if the picked engine is not in `dispatch --engines`, or the one-shot refuses or times out, drop it and fall back to same-engine review — never stall the gate on a missing diverse engine. The exemption covers the **diverse** reviewer only: the same-engine language reviewer and test-runner still run, and having **no** reviewer at all is the terminal path below ("A reviewer you cannot spawn at all is terminal, not a downgrade"), not this fallback.
...
- **Reconcile once.** Merge findings across the batch (both / language-only / diverse-only / security), de-duplicated and checked against the test-runner's deterministic result. Ingest with **receiving-code-review** discipline (Process authority). Fix the real ones (delegate per rule 1), then re-run the **fast deterministic gate**.
...
- **Record the review seam.** When the gate has run (after the last reconcile), post one marker so `crew status … pr_open` finds it: `crew msg "$CREW_WORKER_ID" "review:$(crew id)" '{"seam":"review","review_mode":"<full|downgraded>"}'`. Post it only **after** you have read the verdict(s) — a review request, or a pane that has not yet answered, is not a review. Grid mode's review assignment (lead → `role:<branch>:reviewer`) is never itself a seam; on **pi**, whose `reviewer` pane is the gate, the bus is read in order and the pane's **latest** verdict (a msg from `role:<branch>:reviewer` carrying `"seam":"review"` and `verdict` `accept`/`revise`/`reject`) decides. `accept` passes; `revise` passes only once you have fixed the findings and posted your own `review:<crew>` seam; a `reject` blocks `pr_open` and `done` until the reviewer's next verdict — escalate/block and await the dispatcher, never push past it. A re-request — any lead → reviewer msg other than the bare `{"final":true}` release, even a malformed one or one with neither `seam` nor `artifact` — cancels every earlier verdict **and your own earlier review seam** until a fresh verdict arrives, so do not push between a re-request and its verdict. On pi a reviewer reply that is not a readable exact `accept`/`revise` (an unrecognised verdict, an oversized reply whose verdict was elided, a body that is not a JSON object, an unparsable log line naming this crew and the reviewer, or a `reject`) counts as a reject, so keep the reply small; a reviewer msg carrying a `tag` and no `verdict` (a retro-style note) is not a verdict. Order is what is bound, not the head sha: post-review `/deslop` commits are fine, but re-request review after a behavioural fix (EVIDENCE_REVIEW). A resumed session inherits the bus. The `engine:` stamp in `WORKER_TASK.md` is authoritative at check time and is re-stamped on resume, so a pi → claude resume fails closed to the native seam. On claude, codex and cursor the pane is additive, so the native batch's `review:<crew>` seam is still required and a pane verdict alone does not satisfy the gate. `crew status` **refuses** `pr_open` and `done` on a standard/deep `kind: implement` session until a seam exists for this branch — seams from any earlier session on the branch count, which is why a resumed run needs no override. A seam counts only when sent to `review:<crew>` with `review_mode` absent, `full`, or `downgraded` (or, on pi, the reviewer pane's verdict as above). A retro note never counts.
...
- **Ledger in `pr_open`.** The status detail carries the compact ledger: `crew status "$CREW_WORKER_ID" pr_open "AC1 pass(bats); AC2 pass(nix build); AC3 waived(dispatcher)" <url>`. Each item is `<id> pass(<evidence>)` or `<id> waived(dispatcher)` — the id is one token with no spaces (`AC1`, `AC1-3`); any note goes inside the parentheses (`waived(dispatcher: <note>)`); separate items with `; `. `crew status` refuses a `pr_open` detail with any item not in one of those two forms, on every tier — `pending`, `not run`, `skipped`, `n/a`, `partial`, or a note after the parentheses is refused. It also refuses an empty detail when the task doc has a `## Acceptance` list (empty is accepted only for a task doc with none). `done` is not ledger-checked — its detail is the follow-ups list. Keep it short; the bus clips long lines. The full ledger instead goes in the collapsed `<details><summary>Agent ledger</summary>` block described in "PR body contract" — not a visible `## Acceptance` heading.
...
- if blocked on a question only the dispatcher can answer: post the block, then **await the reply in-band** — don't stop dead:
  ...
  - **times out (empty stdout):** the reply has not arrived yet — **keep waiting in bounded cycles**; do not stop. Fold stragglers first (the bullet below): if the fold returns the **awaited reply**, re-stamp `crew status "$CREW_WORKER_ID" working`, incorporate the answer, and resume the pipeline from where you paused (the reply-arrives bullet above). A **directive** the fold surfaces is not a reply: handle it with **receiving-code-review** discipline (Process authority) — and a conflicting or unclear directive goes block→await (above) rather than a straight resume. Otherwise re-stamp `crew status "$CREW_WORKER_ID" blocked "<why> — awaited 300s, no reply (cycle K of 24)"` — K running 1 to 24, **every** timeout re-stamps, so the roster's `age_s` never goes stale — and start the next 300s cycle, up to a **total wait budget of 24 cycles (~2h)**. The per-cycle re-stamp is mandatory: it is how `crew stall-watch` and the dispatcher see the worker is alive — never skip it. A cycle is one `crew await` bash-tool call; 300s stays inside the 600s tool ceiling, and a parked cycle costs one cheap wake, never a spin loop.
...
7. **Never print a secret, and never read a file whose content is secrets.** No `cat`/`head`/`sed`/`grep` (without `-c`/`-q`) over `.env`, `.env.*`, `.aws/credentials`, `.netrc`, or private keys; no `Read`/`Grep` at them either; no expanding a secret-named variable into output (`echo "${SOME_API_KEY:-x}"` prints the key — a malformed default is the classic way this happens); no bare `env`/`printenv`; and no shell builtin that dumps a variable either — bare `set` (fish and bash/POSIX), fish `set -S`/`set --show`, `declare -p`/`-x`, bare `declare`/`typeset`, `export -p`, `typeset -p`, `/proc/*/environ`, `tmux show-environment`, including inside `fish -c`/`bash -c` — and piping a dump through a filter (`set -S NAME | grep …`) does not make it safe. **The value is never needed:** the tool that consumes it reads the environment itself, and a *missing* key fails loudly — that failure is your signal. To confirm a key is merely present, ask for presence alone — `set -q NAME` (fish) or `[ -n "${NAME:-}" ] && echo set || echo unset` — count without printing (`grep -c '^NAME=' .env`), or just run the tool and read its error. `.env.example` and friends are safe: they hold `op://` references, not values.
   ...
   **If a value does surface:** stop, post a `blocked` status disclosing it, and recommend rotating that credential. Do **not** repeat the value in any message, file, commit, or PR, and do **not** try to scrub your transcript — rotation is the remediation, and re-reading the file to clean it risks a second exposure.
   The repo ships a guard for this rule, `adapters/core/secret-read-guard.sh`. claude and codex run it from the dispatcher plugin's own `PreToolUse` hooks (claude: `Bash`/`Read`/`Grep`; codex: `Bash`, its only read path, once codex trusts the plugin's hooks); cursor runs it only once the README's `~/.cursor/hooks.json` stanza is installed; pi runs it only where hookyard loads this repo's `hookyard.json`. It is a backstop, not the rule: wherever it is not installed, and for any read it cannot parse, this rule *is* the enforcement.
