...
- You **cannot** confirm from source whether the highlight actually has a hole (Trap 5 outcome) or content actually overflows at a given width (Traps 2/3 outcome). When a finding's effect is runtime-only, say so and recommend the verification: a `go build` for compile-class bugs, or a snapshot test that renders to string at a few widths and diffs — the pattern that prevents relapses.
