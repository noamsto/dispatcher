...
1. **Write the artifact** into the crew dir (from `WORKER_TASK.md`):
   ...
   `git diff <base>...HEAD > <crew_dir>/artifacts/<branch>/review.diff`, and the
...
- **Dispatch the review as a single parallel batch** — one message, concurrent subagents — then reconcile once. Do not run reviewers serially. The roles are identical on every engine; the engine only decides how you spawn one, and which rung reviews:
  ...
  | **cursor** | Task-tool subagent with an explicit model slug, the same resolved `brief` inline | the tier's **execute** slug (deep → `cursor-grok-4.6-medium`, standard → `cursor-grok-4.6-low`) |
...
- **Cap the review→fix loop at 2.** Persist unresolved findings under "## Review notes" per `EVIDENCE_REVIEW.md`; its recurrence assessment does not grant additional fix rounds. Pending correctness evidence or review blocks completion rather than allowing an unreviewed last-round fix through.
...
- if blocked on a question only the dispatcher can answer: post the block, then **await the reply in-band** — don't stop dead:
  ...
  - **times out (empty stdout):** re-stamp exactly `crew status "$CREW_WORKER_ID" blocked "await-timeout: <why> — awaited 300s, no reply"` and **stop**. The `await-timeout:` prefix is a reserved, machine-checked marker: emit it exactly, at the start of the detail, and only for this re-stamp — never for the initial `blocked` that precedes `crew await`, never for any other block. By protocol, a worker carrying this marker has ended its turn, which is what makes its pane safe for the dispatcher to wake. Your question stays durable in the bus; when the dispatcher's reply arrives after that, its `crew reply` submits the fixed prompt `crew wake: read your crew inbox and continue` into your pane. On that prompt: re-stamp `crew status "$CREW_WORKER_ID" working`, run the straggler fold (`crew inbox "$CREW_WORKER_ID" --since <seen-cursor>`), handle the reply with receiving-code-review discipline, and resume from the paused step. If the wake prompt arrives and the fold returns nothing new, re-stamp the same `await-timeout:` detail and stop — that is not a new await cycle and does not count toward the cap; it is the same stopping-path instance as the first blocked-timeout stop, so it does not emit another metrics snapshot. **Cap block→await cycles at 2** — after a second timeout, `crew status "$CREW_WORKER_ID" failed "blocked, no dispatcher reply"` and stop.
  - **`trivial`/`standard` only:** if the blocker is low-risk, instead pick a safe default on timeout, document it in the PR body under "## Assumptions", and continue. `deep`/security-sensitive must wait for a real answer. **A missing review gate, pending correctness evidence, or recurrence block is never low-risk**, so these blocks are carved out of this allowance on every tier: it follows block→await→`failed`, and "skip the review" is never a safe default.
...
- **Never use an interactive question tool** (Claude Code's `AskUserQuestion`, or any
  ...
  durable, it wakes the dispatcher, and the dispatcher's reply reaches you in
  place — in-band while you are in `crew await`, and by the wake prompt after
  your turn has ended.
