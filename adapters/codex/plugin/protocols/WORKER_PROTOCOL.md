...
2. **If it trips, pick a consultant and consult.** Judge the fit per task and say which you picked and why (one line, in the plan seam) — neutral fit → **fable**:
   ...
   | **cursor-grok-4.6-high** (work profile) | one-shot in this worktree: `cursor-agent -p --force --trust --approve-mcps --disable-indexing --disable-codebase-ref --model cursor-grok-4.6-high '<prompt>'` — write `DECOMPOSITION.md` yourself from stdout | third-family perspective |
...
- **Scope to changed packages.** Get the changed files with `git diff --name-only <base>...HEAD` (base = the branch's merge-base with the default branch), map each to its module/package, and run build + vet/lint + unit scoped to just those — not the whole repo. Scope is a CPU lever as much as a latency one: you share one machine with sibling workers, and a whole-repo lint saturates every core for all of them.
...
- **Fresh context is the spawn contract, not a style note.** A reviewer subagent receives **only** the task doc, the diff (or the command that computes it), and its role brief. It must **not** receive the plan rationale, the spec, your implementation notes, or any transcript of the execute stage — a reviewer that can see the implementer's reasoning inherits its blind spots and rubber-stamps them. It carries **review authority only**: it does not fix, commit, push, open PRs, or act as the worker (the mirror of rule 1's implementation-authority clamp on execute subagents). And the cheapest hatch is closed by name: **"review it yourself in this context" is not a permitted fallback on `standard`/`deep`** — when no fresh context can be spawned, take the path below instead.
- **A reviewer you cannot spawn at all is terminal, not a downgrade.** This covers having **no fresh-context reviewer whatsoever** — delegation disabled, the spawn refused, the subagent dying before it reports. (A missing **diverse** reviewer is a different thing and stays a should: drop it and review same-engine, per the codex-diverse bullet above.) Retry the spawn **once**. If it still fails, do not push, do not open a PR, and do not emit `none`:
  ...
  This runs on the block→await path in "Report to the bus" (cap 2 cycles, then `failed`). The message must name your **engine and tier**, the **reviewer mechanism attempted**, **how it failed including the one retry**, and the only two legal replies — **retry**, or **re-dispatch** (to an engine that can review, or as `tier: trivial`, where `none` is both legal and true). Say outright that **proceeding unreviewed at this tier is not a legal reply**: that would need a `done` row carrying no honest `review_mode`, which is the bug this gate exists to prevent. Record `review_mode: "unavailable"` with `review_high: null` on the snapshot — `unavailable` appears on a `blocked`/`failed` snapshot only and **never co-occurs with `done`** — and write a `review_unavailable` retro note.
...
| `review_unavailable` | no fresh-context reviewer could be spawned for the code review gate, and the retry failed too. Detail: the engine, the mechanism attempted, and how it failed.                                        |
...
- if blocked on a question only the dispatcher can answer: post the block, then **await the reply in-band** — don't stop dead:
  ...
  - **`trivial`/`standard` only:** if the blocker is low-risk, instead pick a safe default on timeout, document it in the PR body under "## Assumptions", and continue. `deep`/security-sensitive must wait for a real answer. **A missing review gate is never low-risk**, so a `review gate unavailable:` block is carved out of this allowance on every tier: it follows block→await→`failed`, and "skip the review" is never a safe default.
