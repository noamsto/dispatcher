...
Steps 1–3 are the pi path as written. On another engine with panes they are the additive pass: assign the `reviewer` pane once (step 1) and merge its findings into your own set; run step 2's refuter panes over that merged set alongside your native refuters; release both (step 3). Off pi, a pane that stays silent after its one retry is dropped, not blocked on, and the tally merges the native batch's and the panes' `reviewers` and `gaps`.
...
1. **Reviewer.** Write `<crew_dir>/artifacts/<branch>/review.diff` from `git diff "origin/$base...HEAD"` (the validated `base:` above, never a re-derived one) and the resolved roster beside it, exactly as `WORKER_PROTOCOL.md` "Grid mode" step 1 describes (under `lazy: 1`, first `dispatch --spawn-role reviewer`, per that section's lazy-grid paragraph). Assign `{"seam":"review","artifact":"<abs review.diff>","roster":"<abs roster.json>","question":"Review this PR diff; return findings."}` to `role:<branch>:reviewer` and `crew await "$CREW_WORKER_ID" --from "role:<branch>:reviewer" --timeout 300`, bounded exactly as in "Grid mode" step 3. Its reply lists `findings` (`CRITICAL`/`HIGH`/`MEDIUM`, `where`, `what`, `why`), `reviewers` (the roster entries it applied), and `gaps` — the changed files no reviewer covers. Its `verdict` here is `accept` (no findings) or `revise`; it gates nothing.
2. **Refuters.** For each finding, in turn: once a `refuter` pane has answered, `tmux kill-pane` it (find it with the `tmux list-panes` command in "Grid mode" step 3) and `dispatch --spawn-role refuter` a fresh one — the first finding's pane, if freshly created, is already fresh — assign `{"seam":"refute","finding":{"where":"…","what":"…","why":"…"},"question":"Prove this finding wrong."}`, and await it with `--from "role:<branch>:refuter"`. The reply carries `verdict: "confirmed"|"refuted"` and `evidence`; **keep only findings whose reply is exactly `verdict: "confirmed"`**. Any other reply — a `refuted`, a stray `accept`/`revise`, a `role_exited` msg, a missing verdict — is no confirmation: `refuted` drops the finding, anything else counts as no answer, so `tmux kill-pane` it if a live pane remains, `dispatch --spawn-role refuter`, re-send the `refute` assignment (a respawned pane never sees the old one), and await again; one retry only, and if it still gives none take the block→await path above. Never post an unconfirmed finding.
3. **Release.** Once the review is posted, release both roles — `crew msg "$CREW_WORKER_ID" "role:<branch>:<role>" '{"final":true}'` — before the tally. The tally reports `lane: "fan-out"`, the `reviewer` pane's `reviewers`, and its `gaps`.
