...
3. **Await the verdict** — from your bash tool, with a tool timeout above the
   ...
   For the review seam the reply must carry `"seam":"review"` and a `verdict`; on pi that reply is what `crew status`'s review gate reads, and a reject there blocks `pr_open` until the reviewer's next verdict. The assignment in step 2 is only a request, but a new one cancels every earlier verdict and your own earlier seam until a fresh one arrives, so do not push between a re-request and its verdict.
