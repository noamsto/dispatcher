...
On the `review` seam the `seam` and `verdict` fields are what `crew status` counts on pi (the assignment never does), so both must be present; keep the whole reply small — an oversized line is elided leaf by leaf, and a shortened `verdict` no longer counts.
