...
- **Claim (GitHub-issue repos only).** Every issue here is already assigned to the repo owner, so assignee can't signal a claim — the `dispatched` label does instead. An existing-issue dispatch checks that label before touching anything: already there, it aborts naming the issue (no branch/worktree/window); free, `dispatch` adds it before any scaffolding. A minted issue is stamped at creation. `crew reap` removes the label when it reclaims a worker whose PR merged or closed, resolving the issue from the PR's `closingIssuesReferences`. Linear-tracked dispatches are unaffected — Linear has its own status/assignee semantics.
