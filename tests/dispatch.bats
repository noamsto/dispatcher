...
@test "budget gate does not warn when claude quota is known" {
  stub_launch_bins
...
@test "budget gate does not warn about an unknown claude quota in a stale cache" {
  stub_launch_bins
  mkdir -p "$XDG_DATA_HOME/crew"
...
@test "--ignore-budget skips the blind-gate warning" {
  stub_launch_bins
  mkdir -p "$XDG_DATA_HOME/crew"
  jq -n --argjson epoch "$(date +%s)" \
    '{fetched_epoch: $epoch, engines: {claude: null}}' \
