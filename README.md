...
a content hash of `adapters/core/protocols` (minus `PROTOCOL_REV` itself) into
both scripts, and a matching directory carries the same value in a committed
`PROTOCOL_REV` file generated by `scripts/gen-adapters.sh`:

- **built-in default** — always consistent, by construction (same build);
- **override pointing at a checkout** — consistent when the checkout carries a
  generated `PROTOCOL_REV`. After editing a protocol file, regenerate it with
  `./scripts/gen-adapters.sh` (also refreshes the plugin-tree copies); CI's
  drift gate enforces this, and a forgotten regeneration fails the module tests;
- **a stale directory** — an old store path held by a shell/tmux server started
  before a Home Manager switch (the #177 incident), or an old checkout, has an
  absent or older `PROTOCOL_REV` and is **refused** before any scaffolding,
  naming both revisions, the resolved `$PROTOCOL_DIR`, and the override.
